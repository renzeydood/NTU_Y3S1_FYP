...
def compare_humo(target_filtered, set_filtered):
    ...
    for shape in set_filtered:
        set_hu[shape] = cv2.HuMoments(cv2.moments(set_filtered[shape])).flatten()

    for hu in range(0,7):
        target_hu[hu] = -1 * math.copysign(1.0, target_hu[hu]) * math.log10(abs(target_hu[hu]))

    for a_shape in set_hu:
        for a_hu in range(0,7):
            set_hu[a_shape][a_hu] = -1 * math.copysign(1.0, set_hu[a_shape][a_hu]) * m.log10(abs(set_hu[a_shape][a_hu]))
    ...
    for b, c in enumerate(cnts):
        ...
        if cv2.contourArea(c) > 3000 and cv2.contourArea(c) < 10000:
            if ((len(approx) == 8) and compare_withshape(c, shapes_cnts["octagon"], 0.001)):
                ...
                is_red, inner_red = filter_color(img, c, color_set()["red"], 20)
                is_green, inner_green = filter_color(img, c, color_set()["green"], 20)
                ...
                if(is_red):
                    ...
                    if(h[b,2] != -1):
                        print("Child exist")
                        _, ar = (filter_color(img, c, color_set()["white"], 20))
                        ar_c = get_contours(ar)
                        for c_in in ar_c:
                            peri_in = cv2.arcLength(c_in, True)
                            approx_in = cv2.approxPolyDP(c_in, 0.04 * peri_in, True)#0.04
                            print("Traffic light: Green with {} arrow detected".format(findArrow(ar_c, approx_in)))
                            cv2.drawContours(img, [c_in], -1, (240, 0, 159), thickness=2)
                    
                cv2.drawContours(img, [c], -1, (240, 0, 159), thickness=2)
                    
            elif ((len(approx) == 7) and compare_withshape(c, shapes_cnts["arrow_r"], 0.1) and (h[b,3] == -1)):
