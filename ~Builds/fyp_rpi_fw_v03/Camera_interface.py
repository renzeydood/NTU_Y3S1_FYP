...
class Camera_interface(PiVideoStream):
    ...
    def drawShapes(self, shape):
        label = shape.getType() if shape.getOrientation(
        ) == None else shape.getType() + ": " + shape.getOrientation()
        c = shape.getContour()
    ...
    def compareWithColor(self, cnt, clr, thresh):
        ...
        cv2.drawContours(mask, [cnt], -1, (255, 255,
    ...
    def findShapes(self):
        ...
        for b, c in enumerate(cnts):
            ...
            approx = cv2.approxPolyDP(c, 0.03 * peri, True)  # 0.04

            if cv2.contourArea(c) > 1000 and cv2.contourArea(c) < 50000:
                if ((len(approx) == 8) and cv2.contourArea(c) > 2500 and self.compareWithShape(c, self.shapeSet_cnt["octagon"], 0.001)):
                    # if (self.compareWithColor(c, self.clrSet_lab["red"], 20)):
                    #print("Stop sign detected")
                    shaperesult = "Stop " + str(cv2.contourArea(c))
                    cv2.drawContours(
                        self.frame, [approx], -1, (240, 0, 159), thickness=2)

                elif ((len(approx) == 4) and cv2.contourArea(c) > 2500 and self.compareWithShape(c, self.shapeSet_cnt["diamond"], 0.002)):
                ...
                elif (self.compareWithShape(c, self.shapeSet_cnt["circle"], 0.001) and cv2.contourArea(c) > 2500):
                    is_red, inner_red = self.compareWithColor(
                        c, self.clrSet_lab["red"], 20)
                    is_green, inner_green = self.compareWithColor(
                        c, self.clrSet_lab["green"], 20)
                    ...
                    if(is_red):
                        ...
                        if(h[b, 2] != -1):
                            _, ar = (self.compareWithColor(
                                c, self.clrSet_lab["white"], 90))
    ...
    def displayContours(self):
        ...
        for b, c in enumerate(cnts):
            peri = cv2.arcLength(c, True)
            approx = cv2.approxPolyDP(c, 0.03 * peri, True)  # 0.04
            if cv2.contourArea(c) > 200 and cv2.contourArea(c) < 100000:
                cv2.drawContours(
                    self.frame, [approx], -1, (240, 0, 159), thickness=2)
